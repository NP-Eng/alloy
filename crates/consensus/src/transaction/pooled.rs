...
use crate::{
    transaction::{RlpEcdsaTx, TxEip1559, TxEip2930, TxEip4844, TxExtended, TxLegacy},
...
impl PooledTransaction {
    ...
    pub fn encode_for_signing(&self, out: &mut dyn bytes::BufMut) {
        match self {
            ...
            Self::Extended(tx) => tx.tx().encode_for_signing(out),
        }
    }
    ...
    pub const fn as_extended(&self) -> Option<&TxExtended> {
        match self {
            Self::Extended(tx) => Some(tx.tx()),
            _ => None,
        }
    }
    ...
    /// NP TODO
    // NP TODO
    pub fn try_into_extended(self) -> Result<Signed<TxExtended>, Self> {
        match self {
            Self::Extended(tx) => Ok(tx),
            tx => Err(tx),
        }
    }
}
...
impl Encodable2718 for PooledTransaction {
    fn encode_2718_len(&self) -> usize {
        match self {
            ...
            Self::Extended(tx) => tx.eip2718_encoded_length(),
        }
    }
    ...
    fn encode_2718(&self, out: &mut dyn alloy_rlp::BufMut) {
        match self {
            ...
            Self::Extended(tx) => tx.eip2718_encode(out),
        }
    }
