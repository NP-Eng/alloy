...
#[doc(alias = "ExtendedTransaction", alias = "TransactionExtended", alias = "ExtendedTx")]
...
impl Transaction for TxExtended {
    ...
    #[inline]
    fn kind(&self) -> TxKind {
        // NP Rename to sth like Empty / NoAddress
        TxKind::Create
    }
...
pub mod signed_legacy_serde {
    ...
    pub fn serialize<S>(
        signed: &crate::Signed<TxExtended>,
        serializer: S,
    ) -> Result<S::Ok, S::Error>
    where
        S: serde::Serializer,
    ...
    pub fn deserialize<'de, D>(deserializer: D) -> Result<crate::Signed<TxExtended>, D::Error>
    where
        D: serde::Deserializer<'de>,
    {
...
// NP TODO does this even make sense? This structure has no input and therefore
// looks exactly like its re-implementation inside the mod below
...
pub(super) mod serde_bincode_compat {
    ...
    /// ```rust
    /// use alloy_consensus::{serde_bincode_compat, TxExtended};
    /// use serde::{Deserialize, Serialize};
    ...
    /// #[derive(Serialize, Deserialize)]
    /// struct Data {
    ///     #[serde_as(as = "serde_bincode_compat::transaction::TxExtended")]
    ///     header: TxExtended,
    /// }
    /// ```
    ...
    impl From<&super::TxExtended> for TxExtended {
        fn from(value: &super::TxExtended) -> Self {
            Self {
    ...
    impl From<TxExtended> for super::TxExtended {
        fn from(value: TxExtended) -> Self {
            Self {
                chain_id: value.chain_id,
                nonce: value.nonce,
                gas_price: value.gas_price,
    ...
    impl SerializeAs<super::TxExtended> for TxExtended {
        fn serialize_as<S>(source: &super::TxExtended, serializer: S) -> Result<S::Ok, S::Error>
        where
            S: Serializer,
        {
            TxExtended::from(source).serialize(serializer)
        }
    }

    impl<'de> DeserializeAs<'de, super::TxExtended> for TxExtended {
        fn deserialize_as<D>(deserializer: D) -> Result<super::TxExtended, D::Error>
        where
            D: Deserializer<'de>,
        {
            TxExtended::deserialize(deserializer).map(Into::into)
        }
    }
    ...
    mod tests {
        ...
        use super::super::{serde_bincode_compat, TxExtended};
        ...
        fn test_tx_legacy_bincode_roundtrip() {
            ...
            struct Data {
                #[serde_as(as = "serde_bincode_compat::TxExtended")]
                transaction: TxExtended,
            }

            let mut bytes = [0u8; 1024];
            rand::thread_rng().fill(bytes.as_mut_slice());
            let data = Data {
                transaction: TxExtended::arbitrary(&mut arbitrary::Unstructured::new(&bytes))
                    .unwrap(),
            };
...
mod tests {
    ...
    use alloy_primitives::{
        address, b256, hex, Address, FixedBytes, PrimitiveSignature as Signature, B256, U256,
    };

    // NP TODO (no test vectors available)
    // #[test]
    ...
    //         commitment: FixedBytes::<COMMITMENT_BYTES>::default(),
    //     };
